...
import { activityList, comparisonOperators } from '../../../constants/advanceAnalytics';
import { badgesTotalLength } from '../../../constants/varification-badges';

export default function Activity({ handleClose, questStartData }: AddBadgeProps) {
  ...
  const [selectedActivity, setSelectedActivity] = useState({ id: 1, name: 'Posts you’ve created' });
  ...
  const toggleActivityDropdown = () => setActivityIsOpen(!activityIsOpen);
  ...
  return (
    <div className="flex flex-col">
      <h1 className="my-2 text-center text-[10px] font-normal leading-[12px] text-accent-400 dark:text-gray-300 tablet:my-4 tablet:text-[16px] tablet:leading-[16px]">
        Check results for users who added only this number of badges.
      </h1>
      <div className="flex items-center justify-center gap-[15px]">
        <div className="relative inline-block w-full max-w-[120px] tablet:max-w-full">
          <button
          ...
          >
            ...
            <img
              ...
              alt="arrow-right.svg"
              className={`size-[10px] transition-all duration-500 tablet:size-6 ${activityIsOpen ? '-rotate-90' : 'rotate-90'}`}
            />
          </button>
          {activityIsOpen && (
            <ul className="absolute z-10 mt-2 max-h-32 w-fit min-w-[160px] overflow-y-scroll rounded border border-white-500 bg-white text-[10px] dark:border-gray-100 dark:bg-gray-200 tablet:max-h-48 tablet:border-[2px] tablet:text-[20px]">
              {activityList.map((activity) => (
                <li
                  ...
                  onClick={() => {
                    setSelectedActivity({ id: activity.id, name: activity.name });
