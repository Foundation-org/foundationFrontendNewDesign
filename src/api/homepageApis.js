...
export const searchQuestions = async (searchTerm) => {
  if (searchTerm !== '') {
    const response = await api.post(`/search/easySearch`, {
      term: searchTerm,

    });
    return response.data; // Assuming the relevant data is in the response.data property
  }
};
...
export const getAllInCorrect = async (params) => {
    return await api.post(`/infoquestions/getAllQuestsWithIncorrectStatus`,params);
};
